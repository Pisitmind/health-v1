<template>
  <v-container grid-list-lg-xl class="users">
    <div class="">
      <v-card>
        ...
        <v-tabs-items v-model="tab">
          <v-tab-item v-for="data in groups" :key="data.id">
            <v-card flat v-if="data.no === '1'">
              <v-card-content>
                <auth />
              </v-card-content>
            </v-card>
...
export default {
  ...
  data() {
    return {
      tab: null,
      groups: [
        { no: "1", tab: "เจ้าหน้าที่" },
        { no: "2", tab: "ผู้ใช้งานระบบ" },
      ],
    };
  },
};
